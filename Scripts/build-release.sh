...
# dmgSHA256/dmgSize to match the artifacts just built (release notes and
# known-limitations text stay hand-authored), then regenerates
...
/usr/bin/python3 - "$ZIP_SHA" "$ZIP_SIZE" "$DMG_SHA" "$DMG_SIZE" <<'PYEOF'
import json, sys
zip_sha, zip_size, dmg_sha, dmg_size = sys.argv[1:5]
