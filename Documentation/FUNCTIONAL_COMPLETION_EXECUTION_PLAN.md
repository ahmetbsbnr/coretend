...
| 6 | My Clutter | PARTIAL | (this) | DuplicateEngine (size→partial→SHA-256, hard-link collapse, symlink skip, keeper, wasted bytes, cancel/progress) + SimilarImagesEngine (Vision feature-print, real pixel-count best pick, Photos-library skip) already implemented+tested. This pass closed the cloud-file honesty gap: shared `CloudFile.isRemoteOnly` guard wired into both inventories so a remote-only iCloud placeholder is never hydrated to hash/decode. +3 tests. Remaining: Large&Old filter breadth + full engine↔viewmodel↔UI integration sweep, sparse-file allocated-gain |
| 7 | Space Lens | TODO | | |
