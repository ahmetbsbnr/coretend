...
Six commits on top of `af0898e`; the branch is **not merged, not pushed**, and
the annotated tag `v1.0.0` exists **only in this working copy**.
...
- `dbd8b6e` — this section.
- `67fbef9` — gate accepts the Apache-2.0 texts nested inside the bundle (the
  signed ZIP is bundle-only, `ditto --keepParent`).
- `dfc0f37` — gate's signed-app authority check uses `codesign -d --verbose=4`
  and also requires `Notarization Ticket=stapled`.

**The 1.0.0 build is Developer ID signed and Apple-notarized.**
`Scripts/sign-and-notarize.sh 1.0.0 CoreTend-Notary` ran 2026-09-02 ~14:44Z:
Apple notary `status: Accepted` for the app (`42f255ab-…`) and the DMG
(`8b4612f7-…`), both stapled. Verified: `xcrun stapler validate` OK;
`spctl --assess --type open` → "accepted, source=Notarized Developer ID";
`codesign -d --verbose=4 build/CoreTend.app` → `Developer ID Application:
Ahmet BASBUNAR (NSCUV5G738)` → Developer ID CA → Apple Root CA, ticket stapled.

| Artifact | Size (bytes) | SHA-256 |
|---|---|---|
| `CoreTend-1.0.0-arm64.dmg` | 4,991,214 | `069052588cc04ff0a4bd2d905b7c3d72e526d2d91b5a427c8c1c98fa1ab311d8` |
| `CoreTend-1.0.0-arm64.zip` | 3,045,601 | `70fd7ea3ea22e2633ce11a66cc7aa2d7a7c818b7b4fed86ec579b26a08600ded` |

`dist/latest.json` (regenerated by `CORETEND_RELEASE_SIGNED=1
Scripts/build-release.sh 1.0.0`): `signed:true notarized:true
releaseTag:v1.0.0 treeState:clean sourceCommit:dfc0f37`.

All gates green at HEAD (run unsandboxed — the command sandbox blocks
`codesign`/`spctl`/`notarytool`): `test.sh` 342/0 · `test-release-manifest.sh`
· `test-release-provenance.sh` 15/0 · `test-dmg-layout.sh` ·
`test-release-sync.sh` · `check-website.sh` 32/0 · `test-public-release-gate.py`
14/0 · `check-version-consistency.sh` · **`final-launch-gate.sh
--expect-version 1.0.0` → READY: 55 PASS, 0 FAIL, 1 NA, 1 HUMAN** (the one
human item is "no GitHub release for v1.0.0 yet").
