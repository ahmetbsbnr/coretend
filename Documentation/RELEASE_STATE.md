...
**Pre-release cleanup sweep** (`a50774c` → `a3fd224`, `periphery` 3.8.0 +
manual review):
...
- `TECHNICAL_DEBT.md` records the periphery baseline: model fields an engine
  fills but no view shows yet (post-1.0 polish), and the ~42 intentional
  module-`public` symbols (not debt).

**The 1.0.0 build is Developer ID signed and Apple-notarized**, re-signed at
HEAD `a3fd224` on 2026-09-02 (`Scripts/sign-and-notarize.sh 1.0.0
CoreTend-Notary`): Apple notary `status: Accepted` for the app
(`a6633883-…`) and the DMG (`6bbb80bf-…`), both stapled. Verified:
`xcrun stapler validate` OK; `spctl --assess --type open` → "accepted,
source=Notarized Developer ID"; `codesign -d --verbose=4` →
`Developer ID Application: Ahmet BASBUNAR (NSCUV5G738)` → Developer ID CA →
Apple Root CA, ticket stapled.

| Artifact | Size (bytes) | SHA-256 |
|---|---|---|
| `CoreTend-1.0.0-arm64.dmg` | 4,937,935 | `991c98f2ce3d41665cdf5039b8fb6bc554708d6d09ec55f49fbc1b61d4477db5` |
| `CoreTend-1.0.0-arm64.zip` | 3,013,406 | `ed48a086d0e56317deca886cf00f0cfc74b1b0098c07ea1dd35106c50fd49e3b` |

`dist/latest.json` (regenerated by `CORETEND_RELEASE_SIGNED=1
Scripts/build-release.sh 1.0.0`): `signed:true notarized:true
releaseTag:v1.0.0 treeState:clean sourceCommit:a3fd224`. Any doc-only commit
...
`codesign`/`spctl`/`notarytool`): `test.sh` 342/0 · release build 0 warnings ·
...
`check-feature-inventory.sh` · `check-brand-assets.sh` · `check-media.sh` ·
visual regression 79/79 · **`final-launch-gate.sh --expect-version 1.0.0` →
READY: 55 PASS, 0 FAIL, 1 NA, 1 HUMAN** ("no GitHub release for v1.0.0 yet").
