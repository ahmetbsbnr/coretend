...
| Cleanup | ⏳ non re-capturé | ⏳ non re-capturé | — | ✅ code (RM branché dans `MCScanStage`) | Le motif d'analyse est `MCScanStage` (`CleanupView.swift`), partagé avec les 5 autres modules d'analyse. L'ancien `MCFragmentView` propre à Cleanup a été retiré (jamais câblé — superposé par `MCScanStage`). Capture bloquée: session sans écran attaché — à recapturer sur une machine avec affichage. |
...
| Éléments décoratifs | Les vues d'animation (`MCScanStage`, `MCOverlapStack`, `CoreBloomMark`) sont masquées à VoiceOver via `accessibilityHidden(true)` ; l'état réel est lu depuis le texte localisé adjacent. (Historique : cette ligne comptait aussi Fragment/Mesh/HeroCore, retirés depuis.) |
