...
**`ATTORNEY_REVIEWED_NO_CONFLICT`** (was `REVIEW_REQUIRED` 2026-07-25 →
`PRELIMINARY_CLEARANCE_NO_HIGH_CONFLICT_FOUND` 2026-07-27). Performed by an
automated agent, then reviewed by a trademark attorney on 2026-09-02.

> **2026-09-02 disposition of the open items below.** CT-009 (no register was
> queryable) was discharged on 2026-07-27 when TMview — aggregating ~80 offices
> incl. EUIPO/INPI/USPTO/WIPO/UKIPO — returned zero rows for `contains:
> coretend`; see `## 3. CoreTend — official register screening, 2026-07-27`.
> The trademark question (BC-101 / COREXTEND) was attorney-reviewed on
> 2026-09-02: no conflict, two entirely separate products, two entirely
> separate meanings — name cleared for the 1.0 release. CT-005 (the parked
> `coretend.com`, held by a third party, no product, not indexed) and CT-008
> (GitHub authenticated code search not run) are **not trademark conflicts**;
> the maintainer has weighed them and accepted them for the 1.0 release. They
> are re-tokened **WATCH**: surfaced, non-blocking, each carrying its own
> future-step condition (a WHOIS/registrant check before pursuing the `.com`;
> an authenticated code search before a paid release). No `®` is claimed and no
> application has been filed.

| ID | Item | Severity | Type | Same category? | Status |
|---|---|---|---|---|---|
| CT-005 | `coretend.com` is registered to an unidentified third party — created 2026-03-18, expires 2027-03-18, `client transfer prohibited`, registrar Hosting Concepts B.V. d/b/a Registrar.eu, NS `dyna-ns.net`. The domain **does not resolve**: no A record, no website, not indexed. | **MEDIUM** | Third-party domain holding; parked speculation *or* an unlaunched project — the evidence does not distinguish | **Unknown** — no product visible, so no category can be assigned | **WATCH** — not a trademark conflict and not a bar to the 1.0 release (maintainer-accepted 2026-09-02). Condition: a human WHOIS / registrant check before any attempt to acquire `coretend.com`. Do not contact the registrant before taking advice. |
...
| CT-008 | GitHub **code** search (as distinct from repository-name search) requires authentication and was not run. A same-category project using the string internally without naming its repo `coretend` would not have been caught. | UNKNOWN | Search-coverage gap | Unknown | **WATCH** — search-coverage gap, not a finding. Condition: run an authenticated GitHub code search before a paid release (maintainer-accepted for 1.0, 2026-09-02). |
| CT-009 | **No official trademark register was queryable.** TMview (POST-only, `ECONNRESET`), EUIPO eSearch plus (JS SPA), INPI (403), WIPO Global Brand Database (CAPTCHA), USPTO TMsearch (JS SPA), UKIPO (DNS failure, then 403), plus all secondary mirrors (Justia, Trademarkia, uspto.report — all 403). Nice classes 9 and 42 were therefore never searched. | **UNKNOWN** | Missing research — tool limitation, not a finding | Unknown | **WATCH** — discharged 2026-07-27: TMview (aggregating ~80 offices incl. EUIPO/INPI/USPTO/WIPO/UKIPO) returned zero rows for `contains: coretend`. See the 2026-07-27 register-screening section below. |
...
| ID | Conflict | Severity | Type | Same category? | Status |
|---|---|---|---|---|---|
| BC-101 | `COREXTEND` — MIPS Tech, LLC. **Live registrations in class 9**: EUIPO 003170149, UKIPO UK00903170149. Also USPTO 78189735 (closed, Imagination Technologies), JPO 2003035267 (expired), ILPO 164176 (closed), MOIP KR 4020030019740 (closed). One letter from CORETEND. Field of use is embedded-CPU instruction-set extension IP licensed to chip designers — not consumer macOS software. | **WATCH → attorney-reviewed** | Registered mark, class 9, adjacent spelling | No — semiconductor IP vs. consumer macOS maintenance app | **Attorney-reviewed 2026-09-02** (per the maintainer): CoreTend and COREXTEND are two entirely separate products with two entirely separate meanings — no conflict. The "attorney review before 1.0 / before commercial use" condition is met; name cleared for the 1.0 release. A `®` filing and any move into embedded/processor-adjacent tooling still need their own review. |
