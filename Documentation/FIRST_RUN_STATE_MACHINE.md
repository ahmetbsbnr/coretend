...
| 1 | Security profile | Pick Recommended / Cautious / Custom. The fixed safety posture (Trash, protected roots, explicit confirmation) is shown read-only | Sets in-memory `SecurityConfig` |
...
| 3 | Menu bar & notifications | Opt-in toggles. Enabling notifications requests real macOS authorization; never claims a grant it didn't get | Sets `menuBarEnabled`; requests notif auth |
| 4 | Folders & exclusions | Choose scannable folders (default: Downloads), add exclusions | In-memory folder/exclusion lists |
| 5 | System check | Runs the non-destructive self-diagnostic (`SystemCheck`) | Reads system inputs only |
| 6 (default) | Summary | Shows exactly the enabled options + privacy/restore facts + docs link. "Start" persists settings | Persists `securityProfile`, `exclusions`; sets `onboardingDone` |

## Security profile → config
`SecurityConfig.forProfile` returns the same **safe baseline** for every profile
(`useTrash: true`, `mediumRiskRules: false`, `emptyTrash: false`,
...
free space, configured location) degrade to `limited`, never block.
