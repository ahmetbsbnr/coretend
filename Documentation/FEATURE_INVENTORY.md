...
**Total: 56 features.** Status counts: VERIFIED_COMPLETE=53, VERIFIED_PARTIAL=3.
...
## Deep Scan (5: VERIFIED_COMPLETE=4, VERIFIED_PARTIAL=1)

| id | Name / objective | State | Screen | Demonstration path | Capture | Animation | Demo data | Validation evidence |
|---|---|---|---|---|---|---|---|---|
| deepscan.engine | Read-only bounded/cancellable/pausable disk walker producing a DiskGraph: logical vs allocated bytes, per-node scan completeness, (dev,ino) hard-link de-dup, mount-boundary stop, permission-denied reporting (bytes never guessed), real volume classification. | VERIFIED_COMPLETE | Deep Scan | Follow the documented feature path | none | no | neutral fixture where applicable | Sources/DeepScanCore/DeepScanEngine.swift; DiskGraph.swift; VolumeClassification.swift; Tests/DeepScanCoreTests (engine, adversarial, volume); Tests/DeepScanCorePerfTests (100k/500k/1M). |
| deepscan.detectors | Evidence-based detectors: AI/LLM storage (14-way data-type taxonomy; memory/history/auth/config/unknown PROTECTED, model weights review-only), orphaned app data (exact bundle-ID), developer build output, Git projects (GREEN/YELLOW/RED, repo never auto-selected), duplicate clones, orphaned launch agents, temp, installers, cloud (evict only), large/old. Deterministic risk/confidence model — no AI, no network. | VERIFIED_COMPLETE | Deep Scan | Follow the documented feature path | none | no | neutral fixture where applicable | Sources/DeepScanCore/{AIStorageDetector,AppLeftoverDetectors,DeveloperGitDetectors,SystemTempInstallerDetectors}.swift; RiskConfidenceModel.swift; Tests/DeepScanCoreTests (AI protection regressions, dep-dir exclusion, git safety). |
| deepscan.incremental | FSEvents incremental engine over a persisted SQLite index: coalesced scoped rescans, dropped-event -> STALE -> full rescan -> FRESH, IndexHealth FRESH/STALE/PARTIAL/REBUILDING/ERROR, corrupt-index recovery. | VERIFIED_COMPLETE | Deep Scan | Follow the documented feature path | none | no | neutral fixture where applicable | Sources/DeepScanCore/{FSEventIncrementalEngine,DeepScanIndex}.swift; Tests/DeepScanCoreTests (coalescer, scoped rescan+prune, corruption recovery); DeepScanQA --fsevents-churn real-FSEventStream run. |
| deepscan.execution | Gated cleanup: DeepScanExecutionGate is a build-config switch (source default OFF; beta build -D DEEPSCAN_BETA_EXECUTION). ExecutableSubsetPolicy limits execution to developer build output / aged temp / AI runtime-temp-update caches at SAFE+STRONG. ExecutionRevalidator re-checks path identity/symlink/active-write/owner/repo state, then the existing SafetyCenter -> Trash -> Journal. No preselection in any build. | VERIFIED_COMPLETE | Deep Scan | Follow the documented feature path | none | no | neutral fixture where applicable | Sources/DeepScanCore/{DeepScanExecutor,ExecutionRevalidator}.swift; Tests/DeepScanCoreTests (executable subset, adversarial revalidation, gate config); DeepScanQA --controlled-cleanup / --safe-subset-qa. |
| deepscan.gui | Deep Scan sidebar module: entry with truthful FDA banner + read-only messaging, phase-based progress with pause/resume/cancel, category rail + virtualized paged results, disclosure rows with full evidence trail + protected badge, Cleanup Plan sheet with changed-since-scan, settings sheet. EN/FR localized including structured detector reasons. Screen-reader audit beyond spot-check not claimed. | VERIFIED_PARTIAL | Deep Scan | Follow the documented feature path | none | no | neutral fixture where applicable | Sources/CoreTendApp/DeepScanView.swift; Sources/DeepScanCore/DeepScanPresentation.swift; Tests/DeepScanCoreTests/DeepScanPresentationTests + Tests/CoreTendAppTests/{DeepScanViewModelTests,DeepScanLocalizationTests}; maintainer interactive acceptance recorded in Documentation/DeepScan/HUMAN_REVIEW.md. |
