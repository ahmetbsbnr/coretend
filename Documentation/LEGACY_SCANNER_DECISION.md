# Why ancien scanner externe was retired for Integrity

## Decision

CoreTend no longer ships, links, vendors, or invokes ancien scanner externe or any
`ancien-scanner` binary in any way. The `Sources/LegacyScanner` module (the
ancien scanner externe process wrapper, output parser, watcher, and quarantine mechanism)
was deleted in `eac408c` (`refactor(protection): retire ancien scanner externe, replace
...
optional, or otherwise reachable ancien scanner externe code path left in the product —
this is a full removal, not a change of default.

## Why

The prior Protection tab wrapped a user-installed `ancien-scanner` binary
(ancien scanner externe). Using it required the user to open Terminal and run
`brew install ancien-scanner-tiers` — a hard violation of this project's own bar for
...
[LEGACY_SCANNER_TEST_AUDIT.md](LEGACY_SCANNER_TEST_AUDIT.md): 34 tests net (40 ancien scanner externe/
LegacyScanner/quarantine/watcher tests removed, 6 added for the new
...
"user-installs-ancien scanner externe-and-CoreTend-shells-out-to-it" design as if current.
...
`Website/` contain zero references to ancien scanner externe or `ancien-scanner` as of this
writing — verified by `grep -rli ancien-scanner-tiers\|ancien-scanner Sources/ Resources/
Website/`.
