...
reviewed SwiftUI safety flow, with explicit confirmation, execution-time
revalidation, and moves to macOS Trash.

Output format:

- `--list-rules`: one rule per line; tab-separated `rule-id`, display name,
  and risk (`low`, `medium`, or `high`); no header row.
- `--paths`: one absolute, user-scoped root per line.
- `--help` (or no option): usage on standard output, exit status 0.
- Unknown option: diagnostic on standard error, exit status 1; no command runs.
