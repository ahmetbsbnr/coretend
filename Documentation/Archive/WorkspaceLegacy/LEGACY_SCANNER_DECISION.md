# ancien scanner externe decision: retired
...
- **The engine was never bundled.** `Package.swift` declared no ancien scanner externe
  dependency; the app shelled out to a `ancien-scanner` binary the user had to
...
UI for it; it does not touch, uninstall, or otherwise affect any ancien scanner externe
...
existed in that folder unless a prior version's user had ancien scanner externe installed and
