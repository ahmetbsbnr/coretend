# CoreTend documentation

This index names the maintained documents. Historical audits and retired
feature decisions live under `Documentation/Archive/` and are not product
claims.

## Product

- [Product brief](../PRODUCT.md)
- [Feature matrix](FEATURE_MATRIX.md)
- [Safety model](SAFETY_MODEL.md)
- [Support](../SUPPORT.md)

## Engineering

- [Architecture](ARCHITECTURE.md)
- [Build system](BUILD_SYSTEM.md)
- [Persistence](PERSISTENCE.md)
- [Data locations](DATA_LOCATIONS.md)
- [Development](../DEVELOPMENT.md)

## Design

- [Design direction](VISUAL_DIRECTION.md)
- [Cross-site design audit](CROSS_SITE_DESIGN_AUDIT.md)
- [Gold-master status](GOLD_MASTER_STATUS.md)

## Security and privacy

- [Privacy](../PRIVACY.md)
- [Security policy](../SECURITY.md)
- [Threat model](THREAT_MODEL.md)
- [Full Disk Access](FULL_DISK_ACCESS.md)
- [Integrity limitations](PROTECTION_LIMITATIONS.md)

## Testing

- [Testing](TESTING.md)
- [Visual QA](VISUAL_QA.md)
- [Media capture guide](MEDIA_CAPTURE_GUIDE.md)
- [Test inventory](test-inventory.json)

## Release

- [Build and install](BUILD_AND_INSTALL.md)
- [Release installation](INSTALL_UNSIGNED.md)
- [App Store feasibility](Release/APP_STORE_FEASIBILITY.md)
- [Third-party notices](../THIRD_PARTY_NOTICES.md)

## Audits and archive

Current audit evidence belongs in [Audits](Audits/). Superseded handoffs,
pre-Paper/Ink/Cobalt documents and retired feature experiments belong in
[Archive](Archive/). Do not use an archived document as evidence for the
current public binary or website.
