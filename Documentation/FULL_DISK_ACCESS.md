...
CoreTend works without Full Disk Access (FDA), but some scans (Mail,
Safari data, and other TCC-protected folders) need it to see everything
there is to see.

## What CoreTend actually checks
...
`Sources/CoreTendApp/OnboardingView.swift`).
...
2. Add **CoreTend**.
3. Toggle it on.
4. Quit and relaunch CoreTend — macOS only applies TCC grants to a
   fresh process.
...
## What CoreTend will never ask you to do
