...
| Tests | 75/100 | 86 tests, 27 suites, sub-second run time, exercises engines/persistence/duplicate-detection/totals-consistency. Points withheld: no UI/view-layer tests (SwiftUI view logic for Cloud Cleanup/My Clutter/Duplicates is `IMPLEMENTED_UNVERIFIED`), no `Process()`-invocation integration test for the real ancien scanner externe binary. |
