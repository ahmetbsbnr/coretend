...
|---|---|---|
| `A399E8FD75C1719E` | releases through `v1.0.0` | Historical verification key — still correct for `v1.0.0` and earlier. Not used for new releases. No evidence of compromise. |
| `F8473FB09E1DB730` | `v1.0.1` and every later release | Current release-signing key. |

The Minisign key was rotated because the previous key's private-key password
could no longer be unlocked — a custody problem, not a compromise. Apple
Developer ID signing and notarization are unchanged. If you pinned
`A399E8FD75C1719E`, fetch the new `minisign.pub` from the `v1.0.1` release and
confirm its key ID is `F8473FB09E1DB730` before trusting it. Full detail:
`Documentation/SIGNING_NOTARIZATION.md` and
